...
`packages/tokens/tokens/**`는 현재 더미 값으로 채워져 있다. Figma에서 Tokens Studio 플러그인으로 실제 팔레트가 확정되면 GitHub 동기화를 통해 이 폴더에 직접 반영하고, `pnpm --filter @aning/tokens build`로 CSS 변수/타입드 상수를 재생성한다.
